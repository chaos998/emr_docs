...
수가 처방은 5자리 기본 코드만으로 계산이 끝나지 않는다. 진찰료, 주사수기료, 처치료, 검사료 등은 환자 조건과 진료 조건에 따라 산정코드 또는 가산코드가 붙고, 이 값이 최종 금액과 SAM 청구 라인에 영향을 준다.

핵심 원칙:

- 원본 수가코드는 사용자가 선택한 5자리 코드를 보존한다.
- 산정/가산 판단 결과는 별도 필드 또는 파생 처방 라인으로 남긴다.
- 계산은 `원본 수가코드 -> 적용 조건 판단 -> 산정/가산코드 생성 -> 단가/금액 계산 -> 명세서 라인 생성` 순서로 수행한다.
- 화면 표시 금액과 청구 파일 금액은 같은 산정/가산 결과를 사용해야 한다.

```mermaid
flowchart TD
    A[5자리 기본 수가코드 선택] --> B[진료일 기준 수가 마스터 조회]
    B --> C[환자/보험/연령/진료시간/기관 조건 확인]
    C --> D{가산 또는 산정코드 대상?}
    D -- 아니오 --> E[기본 수가코드로 라인 금액 계산]
    D -- 예 --> F[산정코드 또는 가산코드 결정]
    F --> G{처리 방식}
    G -- 산정코드 변경 --> H[기본 라인에 산정코드 저장]
    G -- 별도 가산수가 --> I[파생 처방 라인 생성]
    H --> J[단가/수량/일수 기준 금액 계산]
    I --> J
    J --> K[종별가산/본인부담/청구액 계산]
    K --> L[SAM 처방 라인 생성]
```

| 구분 | 의미 | 처리 방식 |
|---|---|---|
| 원본 수가코드 | 사용자가 선택한 기본 5자리 행위 코드 | 원본으로 보존한다. 수정/삭제 추적 기준이다. |
| 산정코드 | 같은 수가라도 조건에 따라 붙는 세부 산정 구분 | 처방 라인의 산정코드 필드에 저장한다. |
| 가산코드 | 기본 수가와 별도로 산정되는 추가 수가 | 별도 처방 라인으로 생성하거나 자동 산정 라인으로 표시한다. |
| 자동 산정 라인 | 사용자가 직접 넣지 않았지만 계산상 필요한 라인 | 사용자 입력 라인과 구분하고, 원본 라인과 연결한다. |
| 청구 코드 | SAM 명세서에 실제 기재되는 코드/산정정보 | 원본 코드가 아니라 최종 산정 결과 기준으로 만든다. |

대표 조건:

| 조건 | 예시 | 계산 영향 |
|---|---|---|
| 진료 시간 | 야간, 공휴, 토요, 심야 | 진찰료/처치료 등에 가산 산정 |
| 환자 연령 | 1세 미만, 6세 미만, 65세 이상 | 진찰료 코드 선택, 본인부담 계산, 일부 가산 |
| 진료구분 | 초진, 재진, 물리치료, 만성질환관리 | 기본 진찰료 코드 또는 관리료 코드 선택 |
| 의약분업 예외 | 원내 약품 존재, 예외코드 존재 | 조제료, 복약지도료, 의약품관리료 자동 산정 |
| 원내 주사 | 원내 급여 주사 약품 존재 | 주사수기료 자동 산정 |
| 위탁검사 | 외부 수탁기관 의뢰 | 위탁검사 단가/관리료/특정내역 반영 |
| 기관 종별 | 의원/병원/종합병원, 의료급여 여부 | 종별가산 대상 금액 산정 |

### 소스 기준 자동 생성/가산 코드

현재 소스에서 확인되는 자동 생성/가산 코드는 다음 계열이다.

| 계열 | 생성되는 코드 | 생성 조건 | 생성 방식 | 비고 |
|---|---|---|---|---|
| 기본 진찰료 | `AA154` | 접수구분이 신환/초진이거나 마지막 내원일이 초진구분일수를 초과 | 기본 5자리 코드로 생성 | `VisitHistoryToMedicalActionCode`, `VisitTypeToMedicalActionCode` |
| 기본 진찰료 | `AA254` | 접수구분이 재진이거나 기본 재진 판단 | 기본 5자리 코드로 생성 | 기존 코드가 `AA250`이면 만성질환관리료 유지 |
| 물리치료 내원 | `AA222` | 접수구분이 물리치료 | 기본 코드만 생성하고 산정코드 추가 없음 | 물리치료는 즉시 반환 |
| 보호자 대리내원 | `AA254090` | 접수구분이 보호자대리내원 | 재진 코드 `AA254`에 산정코드 `090` 결합 | 5자리 기본 코드 + 3자리 산정코드 형태 |
| 만성질환 | `AA250` | 접수구분이 만성질환 또는 기존 `AA250` 유지 조건 | 기본 5자리 코드로 생성 | 고혈압/당뇨 특례 계산과 연결 |
| 진찰료 연령 산정 | `AAxxx100`, `AAxxx700`, `AAxxx600` | 1세 미만, 1세 이상 3세 미만, 3세 이상 6세 미만 | 기본 AA 코드 뒤에 `computeCode1 + computeCode2 + 0` 결합 | 2017-07-01 이후에는 1~3세 `7`을 `6`으로 통합 |
| 진찰료 시간 산정 | `AAxxx010`, `AAxxx020`, `AAxxx030`, `AAxxx050` | 야간, 6세 미만 심야, 토요 오전, 공휴/일요일 | 기본 AA 코드 뒤에 3자리 산정코드 결합 | 토요 오전은 가산코드 라인과 기본 AA 라인을 함께 생성 |
| 토요 오전 기본 진찰료 | 기본 `AA154`, `AA254`, `AA250` 등 | `computeCode2 == 3` | 산정코드 붙은 라인 외에 기본 AA 라인을 추가 생성 | 토요 오전 가산 검증에서 중요 |
| 추석 연휴 한시 코드 | `IE101` | 건강보험/의료급여, 보험청구 대상, 2025-10-04~2025-10-09 | 별도 자동 수가 라인 추가 | 소스 조건은 `2025-10-03 < 진료일 < 2025-10-10` |
| 추석 연휴 한시 코드 | `IE102` | 건강보험/의료급여, 보험청구 대상, 2025-10-07 하루 | 별도 자동 수가 라인 추가 | 소스 조건은 `2025-10-06 < 진료일 < 2025-10-07` |
| 의약품관리료 | `AL400` | 원내 급여 내복약이 병/팩 단위 단독 | 자동 수가 라인 생성 | 급여약만 대상 |
| 의약품관리료 | `AL801` | 원내 급여 외용약 또는 주사제 단독 | 자동 수가 라인 생성 | 내복약이 없을 때 |
| 의약품관리료 | `AL801`~`AL813` | 원내 급여 내복약 조제일수 1~13일 | `AL8{일수}` 형식 | 예: 3일이면 `AL803` |
| 의약품관리료 | `AL824`, `AL825` | 원내 급여 내복약 조제일수 14~15일 | `AL8{일수+10}` 형식 | 14일은 `AL824`, 15일은 `AL825` |
| 의약품관리료 | `AL836` | 원내 급여 내복약 조제일수 16~30일 | 고정 코드 | 30일 이하 |
| 의약품관리료 | `AL837` | 원내 급여 내복약 조제일수 31일 이상 | 고정 코드 | 장기 조제 |
| 주사수기료 | `KK010`, `KK020`, `KK054`, `KK051`, `KK052`, `KK053` | 원내 급여 주사 약품의 연결 수기료 코드 존재 | 약품 마스터의 `InjectionMedicalActionCode` 기준 자동 생성 | 같은 날 이미 같은 5자리 수기료가 있으면 중복 생성 제외 |
| 주사수기료 연령 산정 | `KK020300`, `KK051300`, `KK052300`, `KK053300` | 2017-07-01 이전, 8세 미만 | 주사수기료 코드 뒤에 `300` 결합 | `KK020/051/052/053` 계열만 대상 |
| 주사수기료 연령 산정 | `KK020100`, `KK020600` 등 | 2017-07-01 이후, 1세 미만 또는 6세 미만 | 주사수기료 코드 뒤에 `100` 또는 `600` 결합 | `KK020/051/052/053` 계열만 대상 |
| 조제/복약지도료 | `J5011` | 의약분업 예외가 있고 원내 내복약이 병/팩 단위 단독 | 자동 수가 라인 생성 | 내복 조제료 계열 |
| 조제/복약지도료 | `J5010`~`J5150` | 의약분업 예외가 있고 원내 내복약 조제일수 1~15일 | `J5{일수}0` 형식 | 예: 3일이면 `J5030` |
| 조제/복약지도료 | `J5116`, `J5121`, `J5126`, `J5131`, `J5141`, `J5151`, `J5161`, `J5171`, `J5181`, `J5191` | 의약분업 예외가 있고 내복약 조제일수 장기 구간 | 구간별 고정 코드 | 16~20, 21~25, 26~30, 이후 10일 구간 |
| 조제/복약지도료 | `J5500` | 의약분업 예외가 있고 외용약 단독 | 자동 수가 라인 생성 | 내복약이 없을 때 |
| 조제/복약지도료 | `J5600` | 의약분업 예외가 있고 내복약과 외용약이 함께 존재 | 자동 수가 라인 추가 | 내복 조제료와 별도 외용 조제료 |
| 위탁검사 관리 | 코드 변경 없음, `OneDay = 1.1` | 위탁검사 대상 검체/병리검사이고 수탁기관이 존재하며 급여 | 수가 라인의 실시 횟수를 1.1로 보정 | 청구 라인은 같은 수가지만 금액이 달라짐 |
| 종별가산 | 별도 코드 없음 | 급여 수가, 위탁검사 아님, 수가 상세의 `OneTwoType == 2` | 금액 합산 시 병원 종별가산율을 곱함 | 2024-01-01 이후 의원 종별가산율은 소스 기본값상 0으로 보임 |

### 산정코드 3자리 구조: ComputeCode1/2/3

수가코드가 5자리보다 길면, 앞 5자리는 기본 수가코드이고 뒤 3자리는 산정코드다. 소스에서는 이 뒤 3자리를 `ComputeCode1`, `ComputeCode2`, `ComputeCode3`로 나눠 다룬다.

```text
최종 수가코드 = 기본수가코드 5자리 + ComputeCode1 + ComputeCode2 + ComputeCode3
예: AA254030 = AA254 + 0 + 3 + 0
```

| 자리 | 위치 | 공통 의미 | 주의 |
|---|---:|---|---|
| 기본수가코드 | 1~5번째 | 행위 자체를 나타내는 기본 코드 | 사용자가 선택하거나 접수구분에서 자동 선택한다. |
| `ComputeCode1` | 6번째 | 주로 연령, 일부 응급/특수 조건 | 코드 계열별로 의미가 다르다. |
| `ComputeCode2` | 7번째 | 주로 시간가산, 일부 응급/특수 조건 | AA 계열에서는 야간/심야/토요/공휴가 들어간다. |
| `ComputeCode3` | 8번째 | 추가 산정, 질가산, 검진 당일 진찰료 사유 등 | AA 기본 진찰료 자동 생성에서는 보통 `0`이다. 검사/병리 질가산에서는 핵심 자리다. |

중요한 원칙:

- 뒤 3자리가 `000`이면 산정코드가 없는 것으로 보고 기본 5자리만 사용한다.
- 산정코드가 붙은 8자리 코드는 기본 5자리 코드와 다른 청구 코드로 취급해야 한다.
- 화면에서는 기본 5자리 코드만 보이더라도, 청구와 계산은 최종 8자리 코드를 기준으로 검증해야 한다.
- `ComputeCode1/2/3`의 의미는 코드 계열별로 다르므로 하나의 공통 enum으로 단정하면 안 된다.

### 기본 진찰료 AA 계열 ComputeCode

기본 진찰료는 `기본 AA 코드 + 3자리 산정코드` 형태로 최종 코드를 만든다. 소스 기준으로 AA 자동 생성에서는 `ComputeCode1`은 연령, `ComputeCode2`는 시간, `ComputeCode3`은 `0`이다.

```text
최종 진찰료 코드 = 기본진찰료코드 + ComputeCode1 + ComputeCode2 + ComputeCode3
AA 계열 자동 생성 = 기본진찰료코드 + 연령산정코드 + 시간산정코드 + "0"
```
...
| 기본진찰료코드 | `AA154` | 초진 |
| 기본진찰료코드 | `AA254` | 재진 |
| 기본진찰료코드 | `AA222` | 물리치료 내원 |
| 기본진찰료코드 | `AA250` | 만성질환관리 |
| 연령산정코드 | `0` | 일반 연령 |
| 연령산정코드 | `1` | 1세 미만 |
| 연령산정코드 | `7` | 1세 이상 3세 미만. 단 2017-07-01 이후에는 `6`으로 처리 |
| 연령산정코드 | `6` | 3세 이상 6세 미만 또는 2017-07-01 이후 6세 미만 |
| 시간산정코드 | `0` | 일반 시간 |
| 시간산정코드 | `1` | 야간 |
| 시간산정코드 | `2` | 6세 미만 심야 |
| 시간산정코드 | `3` | 토요 오전 |
...
| 자리 | 값 | 의미 | 예시 |
|---|---|---|---|
| `ComputeCode1` | `0` | 일반 연령 | `AA254000`은 `000`이므로 실제로는 `AA254` |
| `ComputeCode1` | `1` | 1세 미만 | `AA154100` |
| `ComputeCode1` | `7` | 1세 이상 3세 미만. 2017-07-01 이전 규칙 | `AA154700` |
| `ComputeCode1` | `6` | 3세 이상 6세 미만, 또는 2017-07-01 이후 6세 미만 | `AA154600` |
| `ComputeCode2` | `0` | 일반 시간 | `AA254000`은 기본 `AA254` |
| `ComputeCode2` | `1` | 야간 | `AA254010` |
| `ComputeCode2` | `2` | 6세 미만 심야 | `AA154120` |
| `ComputeCode2` | `3` | 토요 오전 | `AA254030` |
| `ComputeCode2` | `5` | 공휴/일요일 | `AA254050` |
| `ComputeCode3` | `0` | AA 자동 생성 기본값 | `AA254030`의 마지막 `0` |
| `ComputeCode3` | `F`, `G` 등 | 건강검진 실시 당일 별도 진찰료 산정 사유 | 특정내역 `JT018` 기재 대상 |

시간 판정:

| 조건 | 소스 기준 |
|---|---|
| 야간 | 평일 18:00 이후 또는 09:00 이전, 토요일 13:00 이후 또는 09:00 이전 |
| 심야 | 20:00 이후 또는 07:00 이전. 6세 미만일 때 시간산정코드 `2` |
| 토요 오전 | 토요일 08:00 이상 13:00 이하 |
| 공휴 | 일요일 또는 공휴일 마스터에 등록된 날 |

토요 오전은 특이하게 처리된다. `AA254030`처럼 산정코드가 붙은 진찰료를 만들고, 동시에 기본 `AA254` 라인도 추가한다. 따라서 개발 시 토요 오전을 단순히 기본 진찰료의 금액만 바꾸는 방식으로 구현하면 안 된다.

건강검진 실시 당일 별도 진찰료를 산정하는 경우에는 AA 코드의 `ComputeCode3`가 의미를 가진다. 소스에서는 `isSugaAAComputeCode3` 조건이 있으면 줄번호 단위 특정내역 `JT018`을 선택 가능하게 한다. 즉 `ComputeCode3`가 있는 AA 코드는 코드 생성만으로 끝나지 않고, 산정 사유 특정내역까지 같이 관리해야 한다.

### 주사수기료 KK 계열 ComputeCode

주사수기료는 일부 KK 코드만 산정코드를 붙인다.

```text
최종 주사수기료 코드 = KK 원본 5자리 + ComputeCode1 + "00"
```

| 대상 코드 | `ComputeCode1` | `ComputeCode2` | `ComputeCode3` | 의미 |
|---|---|---|---|---|
| `KK010` | 없음 | 없음 | 없음 | 원본 5자리 그대로 생성 |
| `KK054` | 없음 | 없음 | 없음 | 원본 5자리 그대로 생성 |
| `KK020`, `KK051`, `KK052`, `KK053` | `3` | `0` | `0` | 2017-07-01 이전 8세 미만 |
| `KK020`, `KK051`, `KK052`, `KK053` | `1` | `0` | `0` | 2017-07-01 이후 1세 미만 |
| `KK020`, `KK051`, `KK052`, `KK053` | `6` | `0` | `0` | 2017-07-01 이후 1세 이상 6세 미만 |

예시:

| 상황 | 원본 5자리 | 최종 코드 |
|---|---|---|
| 성인 주사수기료 | `KK020` | `KK020` |
| 2017-07-01 이후 1세 미만 | `KK020` | `KK020100` |
| 2017-07-01 이후 6세 미만 | `KK020` | `KK020600` |

### 검사/병리 질가산 ComputeCode3

검사 위탁/수탁기관 품질가산에서는 `ComputeCode3`가 핵심이다. `QualityBonusService`는 기본 5자리 코드 뒤의 3자리 산정코드 중 3번째 자리를 검사 분야와 수탁기관 등급/인증 상태에 따라 바꾼다.

```text
품질가산 최종 코드 = 기본수가코드 5자리 + 기존 ComputeCode1 + 기존 ComputeCode2 + 새 ComputeCode3
```

| `ComputeCode3` 계열 | 의미 | 처리 |
|---|---|---|
| `1`, `2`, `3`, `4` | 진단검사 등급 계열 | 수탁기관 등급 코드로 사용 |
| `H`, `J` | 진단검사 특수 등급 계열 | 진단검사전문의판독이면 `K`, `L`로 변환 |
| `7`, `8` | 핵의학/병리 기본 계열 | 검사 분야 판정에 사용 |
| `A`, `B`, `C`, `D`, `E`, `F`, `K`, `L` | 진단검사전문의판독 변환 코드 | 기본코드로 되돌릴 때는 `6`으로 환원 |
| `G` | 병리과 전문의 외부슬라이드 판독 | 외부슬라이드 여부가 true이고 병리과일 때 사용 |
...
| 현재 `ComputeCode3` | 기본코드로 환원할 때 |
|---|---|
| `A`, `B`, `C`, `D`, `K`, `L`, `E`, `F` | `6` |
| `G` | `9` |
| `1`, `2`, `3`, `4`, `H`, `J`, `7`, `8` | `0` |

이 구조 때문에 검사/병리 가산은 단순히 수가 라인을 하나 더 넣는 문제가 아니다. 기존 코드의 뒤 3자리 중 특히 `ComputeCode3`를 읽고, 기본 코드로 환원하고, 수탁기관 등급을 적용한 뒤 다시 최종 코드를 만들어야 한다.

### 의약품관리료 AL 계열

대상은 원내 급여 약품이다. 원외 약품과 비급여 약품은 제외한다.

| 약품 구성 | 조제일수 | 생성 코드 |
|---|---:|---|
| 내복약이 병/팩 단위 단독 | 무관 | `AL400` |
| 내복약 포함 | 1~13일 | `AL801`~`AL813` |
| 내복약 포함 | 14~15일 | `AL824`, `AL825` |
| 내복약 포함 | 16~30일 | `AL836` |
| 내복약 포함 | 31일 이상 | `AL837` |
| 외용약 또는 주사제 단독 | 무관 | `AL801` |

내복약과 외용약/주사제가 함께 있으면 내복약의 최대 조제일수 기준으로 AL 코드를 선택한다.

### 주사수기료 KK 계열

대상은 원내 급여 주사 약품이다. 약품 마스터의 연결 수기료 코드가 5자리로 존재해야 자동 생성된다.

| 연결 수기료 원본 | 산정코드 대상 | 산정코드 규칙 |
|---|---|---|
| `KK010` | 없음 | 원본 5자리 그대로 생성 |
| `KK054` | 없음 | 원본 5자리 그대로 생성 |
| `KK020` | 있음 | 연령 산정코드 결합 |
| `KK051` | 있음 | 연령 산정코드 결합 |
| `KK052` | 있음 | 연령 산정코드 결합 |
| `KK053` | 있음 | 연령 산정코드 결합 |
...
같은 환자, 같은 진료일에 이미 동일한 5자리 주사수기료가 존재하면 자동 생성하지 않는다. 중복 산정 방지 목적이다.

### 조제/복약지도료 J5 계열

대상은 의약분업 예외가 있는 원내 약품이다. 진료 단위 예외코드가 없으면 생성하지 않는다.

| 약품 구성 | 조제일수 | 생성 코드 |
|---|---:|---|
| 내복약이 병/팩 단위 단독 | 무관 | `J5011` |
| 내복약 포함 | 1~15일 | `J5010`~`J5150` |
| 내복약 포함 | 16~20일 | `J5116` |
| 내복약 포함 | 21~25일 | `J5121` |
| 내복약 포함 | 26~30일 | `J5126` |
| 내복약 포함 | 31~40일 | `J5131` |
...
| 내복약 포함 | 91일 이상 | `J5191` |
| 외용약 단독 | 무관 | `J5500` |
| 내복약 + 외용약 | 무관 | 내복 J5 코드 + `J5600` |

### 위탁검사와 종별가산

위탁검사 관리료는 별도 코드가 추가되는 구조가 아니라 대상 수가의 실시 횟수를 `1.1`로 바꾸는 구조다.

| 구분 | 대상 | 제외 코드 | 처리 |
|---|---|---|---|
| 위탁검사 관리 | `B` 계열 검체검사, 일부 `C` 계열 병리검사 | `B0042`, `B1033`, `B1034`, `B1530`, `B1540`, `C3870` | 위탁검사이고 급여이면 `OneDay = 1.1` |
| `C` 계열 예외 범위 | `C8000`~`C9999`, `CZ976`~`CZ999` | 위 제외 코드 우선 | 위탁검사 관리 대상으로 봄 |

종별가산은 별도 처방코드를 만들지 않고 금액 합산 단계에서 처리한다.

| 조건 | 처리 |
|---|---|
| 처방이 수가가 아님 | 종별가산 제외 |
| 위탁검사임 | 종별가산 제외 |
| 급여 수가이고 수가 상세의 `OneTwoType == 2` | 병원 종별가산율을 곱해 합산 |
| 의료급여 여부 | 병원 종별가산율 선택에 반영 |

필수 저장 데이터:

| 필드 | 뜻 | 사용처 |
|---|---|---|
| 원본 처방코드 | 사용자가 선택한 기본 수가코드 | 수정/삭제, 사용자 화면, 이력 추적 |
| 최종 청구코드 | 산정/가산 적용 후 청구에 사용할 코드 | SAM 처방 라인 |
| 산정코드 | 수가 세부 산정 구분 | 명세서 처방 레코드 |
| 가산구분 | 야간/공휴/토요/심야/소아 등 가산 사유 | 계산 근거, 청구 검증 |
| 자동생성 여부 | 사용자가 직접 입력했는지 자동 생성됐는지 | 화면 표시, 삭제/재생성 판단 |
| 원본 라인ID | 자동 생성 라인이 어느 처방에서 파생됐는지 | 재계산, 원본 삭제 시 동시 삭제 |
| 적용일자 | 수가 마스터와 산정 규칙을 조회한 기준일 | 수가 개정 대응 |
| 계산근거 | 어떤 조건으로 코드가 생성됐는지 | 인증/QA 검증 |
...
- 자동 산정 라인은 사용자가 직접 삭제할 수 있더라도 다음 재계산 시 조건에 맞으면 다시 생성되어야 한다.
- 사용자가 수동으로 추가한 가산 수가와 자동 생성 가산 수가는 중복 산정되지 않아야 한다.
- 청구 파일에는 화면의 기본 5자리 코드만 내보내면 안 되고, 최종 산정/가산 결과가 반영되어야 한다.
- 가산코드는 금액만 올리는 옵션이 아니라 청구 검증 대상 라인이다.
