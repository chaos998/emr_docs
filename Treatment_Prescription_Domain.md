...
| 원본 수가코드 | 사용자가 선택한 5자리 기본 수가코드 | 산정/가산코드가 붙더라도 원본 코드는 보존한다. |
| 최종 청구코드 | 산정/가산 적용 후 청구에 사용할 코드 | SAM 명세서 처방 라인의 코드 기준이 된다. |
...
| 가산구분 | 야간, 공휴, 토요, 심야, 소아 등 가산 발생 사유 | 가산수가 자동 생성과 계산 근거에 사용한다. |
| 자동생성 여부 | 사용자가 직접 넣은 처방인지 시스템이 만든 처방인지 | 자동 산정 수가의 삭제/재생성/화면 표시를 구분한다. |
| 원본 라인ID | 자동 생성 라인이 어떤 원본 처방에서 파생됐는지 | 원본 처방 수정/삭제 시 파생 라인을 같이 재계산한다. |
...
수가코드는 기본적으로 5자리 코드로 선택되지만, 실제 계산과 청구에서는 조건에 따라 산정코드나 가산코드가 붙을 수 있다. 따라서 처방 저장 단계에서 원본 코드와 최종 청구 정보를 분리해야 한다.
...
| 자리 | 저장 의미 | 예시 |
|---|---|---|
| 기본수가코드 | 사용자가 선택한 원본 5자리 코드 | `AA254`, `KK020`, `B...` |
| `ComputeCode1` | 주로 연령 산정 또는 계열별 특수 조건 | AA 1세 미만 `1`, KK 6세 미만 `6` |
| `ComputeCode2` | 주로 시간가산 또는 계열별 특수 조건 | AA 야간 `1`, 심야 `2`, 토요 `3`, 공휴 `5` |
| `ComputeCode3` | 추가 산정/품질가산/검진 당일 사유 등 | AA 기본 `0`, 검사 질가산 `A~L/G` 등 |

처방 저장 시에는 최종 코드 문자열만 저장하면 안 된다. 원본 5자리와 `ComputeCode1/2/3`를 분리해 해석할 수 있어야 수정, 재계산, 특정내역 생성이 가능하다.

| 단계 | 처리 | 저장해야 할 결과 |
|---|---|---|
| 1. 기본 수가 선택 | 사용자가 5자리 수가코드를 선택한다. | 원본 수가코드, 수가명, 기본 단가 |
| 2. 조건 판정 | 진료일자, 진료시간, 환자 연령, 보험구분, 진료구분, 원내약/주사/위탁검사 여부를 확인한다. | 가산구분, 산정 사유 |
| 3. 산정코드 결정 | 같은 기본 코드에 붙는 세부 산정코드를 결정한다. | 산정코드 |
| 4. 가산수가 생성 | 별도 라인으로 청구해야 하는 가산수가가 있으면 자동 생성한다. | 자동생성 라인, 원본 라인ID |
| 5. 중복 방지 | 사용자가 직접 넣은 가산수가와 자동 생성 가산수가가 겹치는지 확인한다. | 중복 제외 여부 |
| 6. 금액 계산 | 기본 라인과 가산 라인을 모두 포함해 금액을 계산한다. | 라인 금액, 종별가산 포함 금액 |
| 7. 청구 생성 | 최종 코드/산정코드/금액을 명세서 처방 라인으로 변환한다. | 최종 청구코드, SAM 처방 라인 |

대표 가산 조건:

| 조건 | 예시 | 처리 |
|---|---|---|
| 진료시간 가산 | 야간, 공휴, 토요, 심야 | 진찰료 또는 해당 수가에 가산코드/산정코드 반영 |
| 연령 가산 | 1세 미만, 6세 미만 | 진찰료 선택 또는 일부 행위 가산 판단 |
| 원내 약품 | 원내 급여 약품 존재 | 의약품관리료, 조제료, 복약지도료 자동 산정 |
| 원내 주사 | 원내 급여 주사 약품 존재 | 주사수기료 자동 산정 |
| 위탁검사 | 외부 수탁기관 의뢰 | 위탁검사 단가/관리료/특정내역 반영 |
| 종별가산 | 기관 종별과 의료급여 여부 | 수가 라인의 종별가산 포함 금액 계산 |

소스 기준 자동 생성 코드:

| 구분 | 자동 생성/변환 코드 | 조건 | 처방 저장 방식 |
|---|---|---|---|
| 초진 진찰료 | `AA154` | 접수구분 신환/초진 또는 마지막 내원이 초진구분일수 초과 | 기본 처방 라인 |
| 재진 진찰료 | `AA254` | 접수구분 재진 또는 기본 재진 판단 | 기본 처방 라인 |
| 물리치료 내원 | `AA222` | 접수구분 물리치료 | 기본 처방 라인, 산정코드 추가 없음 |
| 보호자 대리내원 | `AA254090` | 접수구분 보호자대리내원 | `AA254`에 산정코드 `090` 결합 |
| 만성질환 | `AA250` | 접수구분 만성질환 또는 기존 만성질환 코드 유지 | 기본 처방 라인 |
| 진찰료 연령/시간 산정 | `AA154100`, `AA254010`, `AA254030` 등 | 1세 미만, 6세 미만, 야간, 심야, 토요, 공휴 | 기본 AA 코드 뒤에 3자리 산정코드 결합 |
| 토요 오전 | 산정코드 AA 라인 + 기본 AA 라인 | 토요일 08:00~13:00 | 두 라인이 함께 생성될 수 있음 |
| 추석 연휴 한시 | `IE101`, `IE102` | 2025-10-04~2025-10-09 일부 기간, 건강보험/의료급여 청구 대상 | 별도 자동 처방 라인 |
| 의약품관리료 | `AL400`, `AL801`, `AL8xx`, `AL836`, `AL837` | 원내 급여 약품 구성과 내복약 조제일수 | 별도 자동 처방 라인 |
| 주사수기료 | `KK010`, `KK020`, `KK054`, `KK051`, `KK052`, `KK053` | 원내 급여 주사 약품에 연결 수기료 코드 존재 | 별도 자동 처방 라인 |
| 주사수기료 연령 산정 | `KK020100`, `KK020600`, `KK020300` 등 | `KK020/051/052/053` 계열 + 연령 조건 | 수기료 원본 코드 뒤에 3자리 산정코드 결합 |
| 조제/복약지도료 | `J5011`, `J5xx0`, `J5116`~`J5191`, `J5500`, `J5600` | 의약분업 예외 + 원내 약품 구성/조제일수 | 별도 자동 처방 라인 |
| 위탁검사 관리 | 코드 변경 없음, `OneDay = 1.1` | 위탁검사 대상 검체/병리검사 + 급여 | 기존 수가 라인의 실시 횟수 보정 |
| 종별가산 | 별도 코드 없음 | 급여 수가, 위탁검사 아님, 종별가산 대상 | 라인 금액 또는 합산 금액에 종별가산 반영 |

AA 계열 산정코드:

| 값 | `ComputeCode1` 의미 | `ComputeCode2` 의미 | `ComputeCode3` 의미 |
|---|---|---|---|
| `0` | 일반 연령 | 일반 시간 | 일반 또는 없음 |
| `1` | 1세 미만 | 야간 | 계열별 별도 의미 |
| `2` | 사용 안 함 | 6세 미만 심야 | 계열별 별도 의미 |
| `3` | 사용 안 함 | 토요 오전 | 계열별 별도 의미 |
| `5` | 사용 안 함 | 공휴/일요일 | 계열별 별도 의미 |
| `6` | 6세 미만 | 사용 안 함 | 검사/판독 기본코드 환원값 |
| `7` | 2017-07-01 이전 1세 이상 3세 미만 | 사용 안 함 | 핵의학 계열 |
| `F`, `G` 등 | 사용 안 함 | 사용 안 함 | 건강검진 당일 진찰료 산정사유 또는 검사 질가산 계열 |

검사/병리 질가산은 `ComputeCode3`가 핵심이다. 수탁기관 등급, 검사분야, 외부슬라이드 여부에 따라 뒤 3자리 중 세 번째 자리가 바뀌며, 이 값으로 진단검사/병리/핵의학 계열을 판정한다.

자동 생성 코드 관리 규칙:

- `AL`, `J5`, `KK010`, `KK020`, `KK054`, `KK051`, `KK052`, `KK053` 계열은 소스에서 자동 산정 코드로 분류한다.
- 자동 생성 코드는 사용자가 직접 추가한 처방과 구분해서 저장한다.
- 원내 약품, 주사 약품, 의약분업 예외코드가 바뀌면 기존 자동 생성 코드를 제거하고 다시 생성한다.
- 같은 날 이미 같은 5자리 주사수기료가 있으면 중복 생성하지 않는다.
- 위탁검사는 자동 코드 추가가 아니라 기존 검사 수가의 `1일 횟수`를 1.1로 조정하는 케이스로 분리한다.

운영 규칙:

- 원본 처방을 삭제하면 해당 처방에서 파생된 자동 가산 라인도 삭제한다.
- 원본 처방 조건이 바뀌면 파생 라인을 수정하지 말고 재생성 기준으로 처리한다.
- 자동 생성 라인은 화면에서 구분 표시되어야 하며, 청구에서는 일반 처방 라인처럼 포함된다.
- 가산코드 생성은 DUR보다 먼저 끝나야 한다. 약품 변경으로 자동 조제료/관리료가 바뀌면 DUR 상태도 다시 미확인으로 돌린다.
