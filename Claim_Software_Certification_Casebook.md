...
| 사례 3-1-1 | 6세 이상 65세 미만, 토요가산 | 일반 외래 30% 본인부담, 토요가산 산정/가산코드 |
| 사례 3-1-2 | 6세 이상 65세 미만, 정신요법 | 정신요법 본인부담 경감 |
| 사례 3-1-3 | 65세 이상, 토요가산 | 65세 이상 외래 정액/정률 기준, 토요가산 산정/가산코드 |
...
| 사례 10-1 | 투약량/특수재료/수가 자릿수 | 1회 투약량 소수 자리, T항 특수재료, 산정/가산코드 반영 금액 |
...
| 코드 계열 | 의미 | 주로 걸리는 사례 | 검증 포인트 |
|---|---|---|---|
| `AA154`, `AA254`, `AA222`, `AA250` | 초진/재진/물리치료/만성질환 기본 진찰료 | 사례 3 계열, 사례 11 계열 | 접수구분과 내원 이력에 따라 기본 진찰료가 맞게 선택되는지 확인 |
| `AAxxx100/600/700` | 소아 연령 산정코드 | 사례 3-1-5~3-1-7, 사례 6-1-3, 사례 8-11 | 진료일 기준 연령으로 산정코드가 붙는지 확인 |
| `AAxxx010/020/030/050` | 야간/심야/토요/공휴 산정코드 | 사례 3-1-1, 사례 3-1-3, 사례 11-1, 사례 11-2 | 시간 조건에 따라 산정코드와 본인부담이 같이 바뀌는지 확인 |
| `AL400`, `AL801`, `AL8xx`, `AL836`, `AL837` | 의약품관리료 | 원내조제 사례 3, 6, 8 계열 | 원내 급여 약품 구성과 조제일수에 따라 자동 생성되는지 확인 |
| `KK010`, `KK020`, `KK054`, `KK051`, `KK052`, `KK053` | 주사수기료 | 원내 주사 사례 6-1-2, 8 계열 | 주사 약품의 연결 수기료가 자동 생성되고 중복 생성되지 않는지 확인 |
| `KKxxx100/300/600` | 주사수기료 연령 산정코드 | 소아 주사 처방 사례 | 적용일과 연령에 따라 산정코드가 달라지는지 확인 |
| `J5011`, `J5xx0`, `J5116`~`J5191`, `J5500`, `J5600` | 조제/복약지도료 | 의약분업 예외, 원내조제 사례 | 예외코드와 약품 구성/일수 기준으로 자동 생성되는지 확인 |
| `IE101`, `IE102` | 특정 기간 한시 자동 수가 | 기간 조건 테스트 | 청구 대상 보험진료에서만 생성되는지 확인 |
| 위탁검사 `OneDay = 1.1` | 위탁검사 관리료 성격의 횟수 보정 | 사례 4, 4-1-1 | 별도 코드 추가가 아니라 기존 수가 횟수/금액이 보정되는지 확인 |
| 종별가산 | 급여 수가 종별가산 | 건강보험/의료급여 계산 전반 | 위탁검사는 제외하고 종별가산 대상 수가만 합산되는지 확인 |

산정코드 3자리 검증:

| 구조 | 의미 | 인증 확인 |
|---|---|---|
| 기본 5자리 | 원본 수가코드 | 사용자가 선택한 코드 또는 접수구분에서 자동 선택된 코드가 유지되는지 확인 |
| `ComputeCode1` | 주로 연령 산정 | 1세 미만, 6세 미만, 2017-07-01 전후 규칙이 맞는지 확인 |
| `ComputeCode2` | 주로 시간 산정 | 야간, 심야, 토요 오전, 공휴 조건이 맞는지 확인 |
| `ComputeCode3` | 추가 산정/질가산/검진 당일 진찰료 사유 | `JT018`, 검사/병리 질가산 등 추가 특정내역 또는 코드 변환이 필요한지 확인 |

예시:

| 코드 | 해석 |
|---|---|
| `AA254` | 재진 기본 진찰료. 뒤 3자리 산정코드 없음 |
| `AA254030` | 재진 + 일반 연령 + 토요 오전 + ComputeCode3 없음 |
| `AA154100` | 초진 + 1세 미만 + 일반 시간 + ComputeCode3 없음 |
| `KK020600` | 주사수기료 `KK020` + 6세 미만 산정 |
...
| 사례 3-1-1 | 6세 이상 65세 미만, 토요가산 | 기본 본인부담과 토요가산 반영 | 진료일자와 시간대에 따라 산정/가산코드를 만들고 해당 코드 기준으로 금액을 계산한다. |
| 사례 3-1-2 | 6세 이상 65세 미만, 정신요법 본인부담 경감 | 정신요법 경감 적용 후 본인부담 | 행위 코드의 경감 대상 여부를 수가 속성으로 관리한다. |
| 사례 3-1-3 | 65세 이상, 토요가산 | 65세 이상 본인부담, 토요가산, 면허정보 생략 가능 항목 | 환자 연령 기준일과 진료시간을 기준으로 산정/가산코드를 결정한다. |
...
| 사례 10-1 | 1일 투여량, 총투여일수, 1회 투약량, 특수재료, 산정/가산코드 | 투여량 반올림, 금액 원미만 4사5입, 면허정보, T항 특수재료, 가산 반영 금액 | 수량 계산은 표시값과 계산값의 자릿수 규칙을 분리하고, 가산코드는 금액 보정이 아니라 청구 라인 생성 규칙으로 검증한다. |
...
| 산정/가산코드 엔진 | 5자리 기본 수가코드에서 조건별 산정코드와 자동 가산수가 생성 | 사례 3, 4, 10, 11 |
